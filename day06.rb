...
# character, the character they actually want to send is slightly less likely than the
# others. Even after signal-jamming noise, you can look at the letter distributions in each column
...
positions = Array.new(input.first.size) {|i| Hash.new(0) }
...
# 1st star
puts positions.map {|h| h.max_by{|l,n|n}.first }.join
# 2nd star
puts positions.map {|h| h.min_by{|l,n|n}.first }.join
