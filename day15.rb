...
# before the first disc is reached, and one second elapses as the capsule passes from one disk to
...
parse = /Disc \#(?<disc>\d+) has (?<positions>\d+) positions; at time=0, it is at position (?<initial>\d+)./

discs = []
input.each_line do |line|
  m = parse.match(line)
  discs << { disc: m['disc'].to_i, positions: m['positions'].to_i, initial: m['initial'].to_i }
end

def succeeds?(disc_array, time)
  disc_array.all? {|d| ((time + d[:disc] + d[:initial]) % d[:positions]).zero? }
end

t = 0
until succeeds?(discs, t)
  t += 1
end
...
# Part Two
new_disc = { disc: discs.size + 1, positions: 11, initial: 0 }
discs << new_disc
puts discs.map(&:inspect)
