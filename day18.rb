...
# the next row, using the same rules, and get ^^..^. After determining two new rows, our map looks
# like this:
#
# ..^^.
# .^^^^
...
row = input.chomp.split(//).map {|c| {'.'=>false, '^'=>true}[c] }
safe = row.count {|trap| !trap }

def derived_from(ary)
  from = [nil] + ary.dup + [nil]
  Array.new(ary.size) {|i| from[i] ^ from[i+2] }
end

(part1 - 1).times do
  row = derived_from row
  safe += row.count {|trap| !trap }
end
puts "There are #{safe} safe tiles in #{part1} rows"

part2 = 400000
(part2 - part1).times do
  row = derived_from row
  safe += row.count {|trap| !trap }
end
puts "There are #{safe} safe tiles in #{part2} rows"
