...
# password, is 1.  5017308 produces the next interesting hash, which starts with 000008f82..., so
# the second character of the password is 8.  The third time a hash starts with five zeroes is for
# abc5278568, discovering the character f.  In this example, after continuing this search a total
# of eight times, the password is 18f47a30.
#
...
# For example, if the Door ID is abc:
#
# The first interesting hash is from abc3231929, which produces 0000015...; so, 5 goes in position
# 1: _5______.
...
password = ''
zeroes = '00000'.freeze
hard = Array.new(8, nil)
valid_pos = ('0'..'7').to_a.freeze

until password.length == 8 && hard.count(nil).zero?
  until (md5 = Digest::MD5.hexdigest("#{input}#{index}")).start_with?(zeroes)
    index += 1
  end
  ...
  if password.length < 8
    puts "Found #{pos} from #{md5} at #{index}"
    password << pos
  end

  if valid_pos.include?(pos) && hard[pos.to_i].nil?
    puts "Adding #{md5[6]} at #{pos}"
    hard[pos.to_i] = md5[6]
  end
